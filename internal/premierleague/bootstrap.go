...
type BootstrapData struct {
	...
	TeamIdToName map[int]string
	TeamNameToId map[string]int
}
...
func (bs *BootstrapData) GetTeamName(teamId int) (string, error) {
	if val, ok := bs.TeamIdToName[teamId]; ok {
		return val, nil
	}

	for _, v := range bs.Teams {
		// fmt.Printf("%d-%d\n", teamId, v.Code)
		if v.ID == teamId {
			bs.TeamIdToName[teamId] = v.ShortName
			return v.ShortName, nil
		}
	}
...
func (bs *BootstrapData) GetGameWeekByOffset(offset int) (Gameweek, error) {
	for k, v := range bs.Gameweeks {
		if v.IsCurrent {
			index := k + offset
			if index >= 0 && index < len(bs.Gameweeks) {
				return bs.Gameweeks[index], nil
			}
			break
		}
	}
